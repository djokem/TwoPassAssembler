...
void Asembler::popuniMapuNaredbi(){
 ...
 int Asembler::instrukcije(string linija) {
	 ...
	 if (!pom.empty()) {
		 ...
		 Naredba* n = new Naredba(mn, this->sve_naredbe[mn], op1, op2, adr1, adr2, trenutnaSekcija);
		 n->setInstrLocCnt(this->loc_cnt);
 ...
 int Asembler::drugiProlaz(ifstream& fajl) {
	 ...
	 if (fajl.is_open()) {
			 ...
			 if (!pom.empty()) {
				 ...
				 while (stok != NULL) {
					 if ((s = this->tabelaSimbola->getSimbol(stok)) != 0)
						 ...
						 this->tabelaSimbola->putSimbol(new Simbol(stok, this->trenutnaSekcija, true, -9999), stok);
					 stok = strtok(NULL, " ,\r");
				 }
				 continue;
			 }
		 ...
		 map<int, Instrukcija*>::iterator itt;
		 for (itt = this->naredbe.begin(); itt != this->naredbe.end(); itt++) {
			 Instrukcija* s = itt->second;
			 if (Naredba * n = dynamic_cast<Naredba*>(s)) {
				 n->updateOp1Im(n->getAdrOp1(), n->getVelicina1(), n->getOp1p(), tabelaSimbola, tabelaSekcija, n->getOp1Im1(), n->getOp1Im2(), true);
				 n->updateOp1Im(n->getAdrOp2(), n->getVelicina2(), n->getOp2p(), tabelaSimbola, tabelaSekcija, n->getOp2Im1(), n->getOp2Im2(), false);
			 }
		 ...
		 for (it = tabelaSekcija->mapa.begin(); it != tabelaSekcija->mapa.end(); it++) {
			 Sekcija* s = it->second;

			 fajlOut << "\t\t\tIZLAZNI KOD " << s->getIme() << " SEKCIJE" << endl << endl;
			 map<int, Instrukcija*>::iterator it;
			 for (it = this->naredbe.begin(); it != this->naredbe.end(); it++) {
				 Instrukcija* ins = it->second;
				 string s1 = ins->getSekcija();
				 string s2 = s->getIme();
				 if (!s1.compare(s2)) {
					 if (Naredba * n = dynamic_cast<Naredba*>(ins)) {
						 unsigned in = (unsigned)n->getOC();
						 in = in<< 24;
						 in = in >> 24;
						 fajlOut << hex << in << " ";
						 int k;
						 ...
						 unsigned in_1;
						 unsigned in_2;
						 if ((k = n->getVelicina1()) > 0) {
							 switch (k) {
							 case 1:
								 in_0 = (unsigned)n->getOp1Descr();
								 in_0 = in_0 << 24;
								 in_0 = in_0 >> 24;
								 fajlOut << hex << in_0 << " ";
								 break;
						 ...
						 if ((k = n->getVelicina2()) > 0) {
							 switch (k) {
							 case 1:
								 in_0 = (unsigned)n->getOp2Descr();
								 in_0 = in_0 << 24;
								 in_0 = in_0 >> 24;
								 fajlOut << hex << in_0 << " ";
								 break;
					 ...
					 else if (Direktiva * d = dynamic_cast<Direktiva*>(ins)) {
						d->razresiSimbole(tabelaSimbola, tabelaSekcija);
						for (vector<int>::iterator it_in = d->parametri.begin(); it_in != d->parametri.end(); it_in++) {
							unsigned a = (unsigned)*it_in;
							if (d->getVelicina() == 2 && d->mnemonik != ".skip") {
								unsigned b = a;
								a = a << 24;
								a = a >> 24;
								b = b << 16;
		 ...
		 fajlOut << endl << "\t\t\tRELOKACIONI ZAPIS " << endl << endl;
		 for (it = tabelaSekcija->mapa.begin(); it != tabelaSekcija->mapa.end(); it++) {
			 Sekcija* s = it->second;
			 for (vector<RelokacioniZapis>::iterator itr = s->relzap.begin(); itr != s->relzap.end(); itr++) {
				 RelokacioniZapis r = *itr;
				 fajlOut << r.toString() << endl;
			 }
		 }
...
int Asembler::prviProlaz(ifstream& fajl) {
    ...
    if (fajl.is_open()) {
        while (getline(fajl, linija)) {
            ...
            if (!pom.empty()) {
				...
				Direktiva* d = new Direktiva(pom.str(), trenutnaSekcija);
                linija = pom.suffix().str();
	...
	tabelaSekcija->getSekcija(this->trenutnaSekcija)->setKraj(this->loc_cnt);
